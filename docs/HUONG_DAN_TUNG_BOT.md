...
- **Tắt hẳn 1 tài khoản** thì làm trên sheet tổng: để **Bật = N** — bot tự dừng tài khoản đó
  trong `config_reload_seconds` (mặc định 60 giây), **không cần đổi ô B1**. Bật lại `Y` là tự mở.
...
| Cấu hình | Mặc định | Ý nghĩa |
|---|---|---|
| `cancel_orders_seconds` | 3600 | Bao lâu quét 1 lần (**giây**) — khai cả hai thì dòng giây thắng |
...
thì bot tự nhận trong `config_reload_seconds` — **không cần đổi ô B1**.
